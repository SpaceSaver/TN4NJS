            ...
            file_url_holder_req.on("response", res => {
                ...
                res.on("end", () => {
                    if (Math.floor(res.statusCode / 100) == 2) {
                        ...
                        place_file_req.on("response", pres => {
                            ...
                            pres.on("end", () => {
                                if (Math.floor(pres.statusCode / 100) == 2) {
                                    ...
                                    reject(`Received a non-200 response code. (${pres.statusCode}) (place file)\n${JSON.stringify(headers_place_file)}`);
                                }
                            });
                        });
                        ...
                        reject(`Received a non-200 response code. (${res.statusCode}) (file holder)`);
                    }
                });
            });
